...
def wr_grn_cr(pos_shft, *cfg):
    """
    CVC equiv crowm
    """
    # interp cfg parameter
    interp_df = pd.read_excel(
        "cfg_crlc/wr_grn_cr_interp_vec_%d.xlsx" % cfg[0]["line"]
    )
    # config_file
    profile_df = pd.read_excel(
        "cfg_crlc/profile_df_%d.xlsx" % cfg[0]["line"]
    )
...
def Crns(input_df, *cfg):
    ...
    wrbr_df = pd.read_excel(
        "cfg_crlc/wrbr_para_%s.xlsx" % cfg[0]["line"])
...
if __name__ == '__main__':
    ...
    print(wr_grn_cr(pos_shft_array, cfg))
