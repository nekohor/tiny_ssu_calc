...
class CompositeRollStackCrown(object):
    ...
    def __init__(self, fsstd, crn_stack=None):
        self.fsstd = fsstd

        if crn_stack is None:
            self.crn_stack = pd.DataFrame()
        else:
            self.crn_stack = crn_stack

        self.std_vec = np.array([1, 2, 3, 4, 5, 6, 7])

        self.cfg_prof = pd.read_csv("profile.csv")
        self.cfg_interp = pd.read_csv("wr_grn_cr_interp.csv")
        self.cfg_wrbr = pd.read_csv("wrbr_para.csv")
