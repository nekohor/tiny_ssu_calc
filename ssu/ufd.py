...
class UniForcDist():
    ...
    def init(self):
        ...
        for std in self.std_vec:
            ...
            self.bmtrx.loc[0, std] *= dp_dfrcw_mul
            self.bmtrx.loc[1, std] *= dp_dfrcw_mul
            self.bmtrx.loc[2, std] *= (dp_dpcwr_mul * pow(br_len / wr_wid, 2))
            self.bmtrx.loc[3, std] *= (
                dp_dfrcw_mul * dp_dwrbr_mul * pow(br_len / br_wid, 2))
            self.bmtrx.loc[4, std] *= (
                dp_dfrcw_mul * dp_dwrbr_mul * pow(br_len / br_wid, 2))
            self.bmtrx.loc[5, std] *= dp_dbnd_mul
            self.bmtrx.loc[6, std] *= dp_dfrcw_mul * dp_dbnd_mul
            self.bmtrx.loc[7, std] *= dp_dfrcw_mul * dp_dbnd_mul
            self.bmtrx.loc[8, std] *= dp_dwrbr_mul * pow(br_len / br_wid, 2)
            self.bmtrx.loc[9, std] *= avg_diam_wr * dp_dfrcw_mul
            self.bmtrx.loc[10, std] *= avg_diam_wr * dp_dbnd_mul
            self.bmtrx.loc[11, std] *= avg_diam_br * dp_dfrcw_mul
            self.bmtrx.loc[12, std] *= equiv_mod_wr * dp_dfrcw_mul
            self.bmtrx.loc[13, std] *= equiv_mod_wr * dp_dbnd_mul
            self.bmtrx.loc[14, std] *= (
                avg_diam_wr * dp_dpcwr_mul * pow(br_len / wr_wid, 2))
            self.bmtrx.loc[15, std] *= (
                equiv_mod_wr * dp_dpcwr_mul * pow(br_len / wr_wid, 2))
            self.bmtrx.loc[16, std] *= avg_diam_wr * avg_diam_br
            self.bmtrx.loc[17, std] *= avg_diam_br * equiv_mod_wr
